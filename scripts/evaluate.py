...
local FAISS index via SOPSearcher, not on any hosted model API. So there's no
...
import numpy as np
from search import SOPSearcher  # Your retriever. Must expose .search(), .metadata, .query_cache


class QuokkaEvaluator:
    """
    Wraps a single SOPSearcher and runs evaluation/benchmark routines against it.
    ...
    def benchmark_performance(self, num_queries: int = 100) -> Dict:
        ...
        for i, chunk in enumerate(sample_chunks, 1):
            ...
            # CPU or GC pause can flip the result. If SOPSearcher can expose a
    ...
    def compare_configurations(self) -> Dict:
        ...
        for name, config in configs.items():
            ...
            # coupled to SOPSearcher's internals. If that attribute is ever
